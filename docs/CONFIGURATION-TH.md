...
> ⚠️ เมื่อเปลี่ยน Port ให้รันด้วย `npm start` ตรง ๆ อย่าใช้ `scripts/start-windows.ps1` เพราะสคริปต์นี้ตรวจความพร้อมที่ `http://127.0.0.1:4173` แบบตายตัว มันจะรอไม่เจอ แล้วปิดเซิร์ฟเวอร์เองหลัง 10 วินาที (ตัวเปิดของเวอร์ชัน Setup.exe อ่านค่า `PORT` ได้ปกติ แต่ `HR_AUTO_DATA_DIR` ถูกบังคับเป็น `%LOCALAPPDATA%\HR-Auto\data` เสมอ)
